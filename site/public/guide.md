...
There is one current model-routing compatibility detail: the child allowlist forwards the legacy `DELTA_MODEL` variable, but not `DELTA_MODEL_PRIMARY`. If the parent model differs from Delta's default, set both variables to the same value when using subagents. The parent honors `DELTA_MODEL_PRIMARY`; the child reads the forwarded `DELTA_MODEL` alias. The child also does not receive `MODEL_HEADERS`, `DELTA_MODEL_PRICES`, or reasoning settings, so a custom-header route, custom metering, or parent reasoning configuration is not reproduced in the child.
