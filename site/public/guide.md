...
Below 120,000 the retained tail scales with the ceiling (20% of it, floored at 6,000 tokens, capped at the usual 24,000) instead of staying flat. A flat 24,000-token tail is 12% of a 200,000 window but 40% of a 60,000 one, and at that size the tail alone re-crossed the ceiling within two or three turns: measured on a real recruiting run, 0.2.16 at a 60,000 ceiling compacted 25 times in 62 turns and re-ran the same searches after every cut. At 120,000 and above nothing changes.
...
- `recall` searches this conversation's earlier turns, including ones compacted out of the active window. Any word of the query qualifies a row (function words are dropped), rows rank by how many query words they carry and then by recency, and a whole-phrase match ranks first, so "Maria Delgado at Acme" finds the row that says "Maria Delgado, Acme Robotics". It returns the matching finding — the whole message when it is reasonably sized, not a fragment — along with the on-disk path of any large result that was spilled. On identical questions drawn from production compactions, this search answered 53% where the old whole-phrase match answered 21%, and 72% on the facts the agent itself had written down. Searches are scoped to the newest 5,000 rows of the current session; a caller cannot search another session's transcript.
