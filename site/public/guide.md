...
in the built-in table, and so is GPT-6 Astra (`gpt-6-astra`, since 0.2.18): same wire, explicit cache marks on
`api.openai.com` (the Codex backend refuses the field for this model, so a subscription lane runs
implicit caching), reasoning effort `low` through `max` (`none` and `minimal` are refused by the
model and named at boot). Since 0.2.16, an `api.openai.com` Responses lane is a first-class
...
- **Explicit prompt-cache breakpoints** (GPT-5.6 and GPT-6 on `api.openai.com`). The same placement engine that drives
