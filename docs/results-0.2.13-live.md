...
| problem | fix |
|---|---|
| `calls` bounded by nothing | `pruneLocalState` bounds it by age plus a **byte** budget (`DELTA_RETENTION_MAX_CALL_BYTES`, 32MB). Bytes not rows, because a captured call is ~95KB on one lane and ~700KB on another. The newest call is always kept even when it alone exceeds the budget, so the bound can never discard the turn being debugged. **Corrected 2026-08-10 (codex):** the first version measured with SQLite `LENGTH()`, which counts CHARACTERS, so a knob named `..._MAX_CALL_BYTES` enforced a character budget and undercounted a non-ASCII payload by up to ~3x. Now `LENGTH(CAST(... AS BLOB))`, regression-tested with a multi-byte payload. The live end-to-end verification did not catch this because the test data was ASCII. |
| ~~`.delta/spill` bounded by nothing~~ | **REVERTED the same day, and the item is still open.** `sweepSpill` (boot-only, 7-day mtime TTL) would have destroyed recoverable output on any lane depending on cross-run `recall`. The reference holders are the transcript, compaction's accumulated pointers and `recall` itself — not the `journal`/`events` horizons I reasoned against. `queue.ts` already carried a comment saying durable spill must survive across runs, at the one place that deletes it. See `shipping-list.md`. |
