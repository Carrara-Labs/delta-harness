...
async function runInstrumented(effort: "low" | "medium", rep: number) {
  ...
  const messages: ChatMsg[] = [
    { role: "system", content: SPINE },
    {
      role: "user",
      content:
        "Find the founder and current funding of Helix Dynamics, the robotics company in Munich. Save me a brief, then tell me what you found.",
    },
  ];
  ...
  for (let round = 0; round < 7 && !(saved && calls > 0); round++) {
    ...
    for (const tc of r.message.tool_calls) {
      if (tc.function.name === "qs_save_artifact") {
        saved = true;
        messages.push({ role: "tool", tool_call_id: tc.id, content: "saved: brief.md" });
      } else {
        sawSearch = true;
        messages.push({
          role: "tool",
          tool_call_id: tc.id,
          content: searchRound === 0 ? ROUND1 : ROUND2,
        });
      }
    }
    if (sawSearch) searchRound++;
    if (saved) {
      const rf = await chat(cfg, {
        messages,
        tools: TOOLS,
        cacheKey,
        maxTokens: 8000,
